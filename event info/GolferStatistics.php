...
,TG.strFirstName
order by 
TGYE.dtmDateOfSponsor ASC

";

if($result = mysqli_query($conn, $sql2)){
    if(mysqli_num_rows($result) > 0){
    echo"  <h2 align='center' class='golfers'>Golfers total raised & Team</h2>";
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        //  {
          ...
          }elseif ($row['amount_pledge_for_Golfer'] < 1000 ){
            $style = 'style="color:black ;"';
          }
            echo "<td  $style >" . $row['intGolferID'] . "</td>";
            echo "<td  $style>" . $row['strLastName'] . "</td>";
            echo "<td $style>" . $row['strFirstName'] . "</td>";
            echo "<td  $style>" . $row['strTypeofTeam'] . "</td>";
            echo "<td  $style>" . $row['amount_pledge_for_Golfer'] . "</td>";
            echo "<td><a href='ViewDonors.php?intGolferID=" . $row['intGolferID'] . "'> View Donor</a> </td>";
        echo "</tr>";
        //  }
    }
    echo "</table>";
